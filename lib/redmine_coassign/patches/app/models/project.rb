	...
	module ProjectPatch
		class << Project
			def visible_condition_with_coassignees(user=User.current, options={})
				...
				issues = Issue.joins(:custom_values).where('custom_values.customized_type' => 'Issue', 'custom_values.custom_field_id' => Setting.plugin_redmine_coassign['coassign_custom_field_id'].to_i, 'custom_values.value' => current_uid)
		...
		def self.included(base)
			...
			base.class_eval do
				...
				def visible_with_coassignees?(user=User.current)
					...
					issues = Issue.joins(:custom_values).where('custom_values.customized_type' => 'Issue', 'custom_values.custom_field_id' => Setting.plugin_redmine_coassign['coassign_custom_field_id'].to_i, 'custom_values.value' => current_uid, :project_id => self.id)
